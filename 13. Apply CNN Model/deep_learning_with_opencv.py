...
#load the class labels from the disk
rows = open(args["labels"]).read().strip().split("\n")
classes = [r[r.find(" ") + 1:].split(",")[0] for r in rows]

#our cnn requires fixed spatial dimensions for our input images
#so we need to ensure it is resized tpo 224x224 pixels while performing
#mean subtraction(104, 117, 123) to normalize the input
#after executinhg this command our "blob" now has the shape:
#(1, 3, 224, 224)
...
#set the blob as the input to the network and perform a forward-pass to 
